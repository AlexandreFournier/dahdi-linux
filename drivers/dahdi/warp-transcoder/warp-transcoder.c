...
{	
	...
	if (likely(dahdi_warp_receive_packet_payload(DEFAULT_DSP_ID,
		...
		if (printk_ratelimit()) {
			dahdi_warp_log(DSP_TCODER_LOG, "%s() : Error Reading Packet Payload for Channel (%d)\n",
				__FUNCTION__, pvt->chan_num);
		}
		ret = 0;
	}
...
__init dahdi_warp_transcoder_init(void)
{
	...
	numchannels = dahdi_warp_get_num_g729_channels(DEFAULT_DSP_ID);
	// Asterisk does not load the transcoder if only 1 channel is reported (MI9799).
	if (numchannels == 1) numchannels = 2;
	wtc->numchannels = numchannels;
	
	//init_MUTEX(&wtc->chansem);
	sema_init(&wtc->chansem,1);
