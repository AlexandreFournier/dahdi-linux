...
static u32 swap_address( u32 val )
{
	u16 *v; 
	u32 r;
	v = (u16*)&val;
...
static void ReadCircStruct (ADT_UInt32 DspId, DSP_Address CircBufrAddr, CircBufr_t *cBuff,
	...
	cBuff->BufrBase = swap_address(cBuff->BufrBase);

    //printk("ReadCircStruct: dspid: %d, CircBufrAddr: %x, CB_INFO_STRUCT_LEN: %d\n", DspId,CircBufrAddr, CB_INFO_STRUCT_LEN);
...
void ReadCircBuffer (ADT_UInt32 DspId,  CircBufr_t *cBuff, DSP_Word *hostBuff,
	...
	//printk("ReadCircBuffer: dspid: %d, BurfrBase: %x TakeIndex:%x  TakeAddr: %x - cBuffAddr %x\n",DspId, cBuff->BufrBase, cBuff->TakeIndex, TakeAddr, cBuffAddr);
...
int CheckDspReset (ADT_UInt32 DspId) {
    ...
    if ((DspStatus == DSP_INIT_STATUS) ||
       ((DspStatus == HOST_INIT_STATUS) && (pDspIfBlk[DspId] == 0))) {
		//printk("CheckDspReset (dspid: %d): DspStatus: %x pDspIfBlk[DspId]: %x\n", DspId, DspStatus, pDspIfBlk[DspId]);
       ...
       /* DSP was reset, read the DSP's interface parameters (CmdMsgLen and MaxChannels)
		...
		//pr_err("IfBlockPntr + EVENT_MSG_PNTR_OFFSET: %x + %x\n",IfBlockPntr , EVENT_MSG_PNTR_OFFSET);
...
static ADT_UInt32  TransactCmd (ADT_UInt32 DspId,         ADT_UInt32 *pMsgBufr,
    ...
    while ((FuncStatus = WriteDspCmdMessage (DspId, pMsgBufr, ElemCnt)) != 1)   {
        ...
        gpakHostDelay(DspId,0);	// delay until the write buffer is available
    }
    ...
    if (FuncStatus == 1)  {  // Successful command transfer to DSP
        ...
        for (LoopCount = 0; LoopCount < MaxWaitLoops; LoopCount++)  {
            ...
            if (FuncStatus == 0)  {
               gpakHostDelay(DspId, 1);	// delay until the read buffer is available
               continue;  // Retry
            }
...
gpakReadEventFIFOMessageStat_t gpakReadEventFIFOMessage (ADT_UInt32 DspId,
	...
	memset(&cBuff, 0, sizeof(CircBufr_t));
	// Read circular buffer structure into host memory
...
//	printk("Dump ChannelId %x (%d): EvtCode %x (%d): EvtDatabytes %x (%d): ToneDevice: %x (%d)\n", 
//			EvtBuffer[0],EvtBuffer[0], EvtBuffer[1],EvtBuffer[1],EvtBuffer[2],EvtBuffer[2],EvtBuffer[3],EvtBuffer[3]);

    if (EVENT_BUFFER_SIZE < EvtDataWds) {
	   PurgeCircStruct (DspId, EvtAddr, &cBuff);
       gpakUnlockAccess (DspId);
	   printk("Dump %x:%x:%x:%x\n", EvtBuffer[0],EvtBuffer[1],EvtBuffer[2],EvtBuffer[3]);
	   printk("Error Line: %d DspId: %d EVENT_BUFFER_SIZE: %d EvtDataWds: %d:%d\n", __LINE__,DspId, EVENT_BUFFER_SIZE, EvtDataWds,EvtDataBytes);
       return (RefInvalidEvent);
    }

	printk("Dsp: %d EventCode: %d\n", DspId, EvtCode);
    EvtError = 0;
    switch (EvtCode) {
...
gpakDownloadStatus_t gpakDownloadDsp (ADT_UInt32 DspId, void *FileId) {
 ...
 #error DSP_TYPE must be defined
        ...
        while (WordCount != 0)      {
			...
			/*
			printk ("-----------------------------\n");
            printk ("gpakDownload: %x - %d \n", (unsigned int)Address, stat);
			print_hex_dump_bytes("DlWordBufr:", 1,DlWordBufr,BlockSize);
			print_hex_dump_bytes("DlByteBufr:", 1,DlByteBufr,BlockSize);
			*/
            if (stat != 0) {
               RetStatus = GdlFileReadError;
...
static void transferDlBlock_be (ADT_UInt32 DspId, DSP_Address Address, unsigned int NumWords) {
	...
	//print_hex_dump_bytes("DUMP DLBYTESBUFFER - ", 2,DlByteBufr, NumWords);
    // Big endian mode does the swap in the Write and Read routiner
    gpakWriteDsp (DspId, Address, NumWords, (DSP_Word *) DlByteBufr);
    gpakReadDsp  (DspId, Address, NumWords, (DSP_Word *) DlWordBufr);
	//print_hex_dump_bytes("DUMP DLWORDBUFFER - ", 2,DlWordBufr, NumWords);
}
static void transferDlBlock_le (ADT_UInt32 DspId, DSP_Address Address, unsigned int NumWords) {
...
unsigned int i;
	...
	//print_hex_dump_bytes("DUMP DLBYTESBUFFER LE - ", 2,DlByteBufr, NumWords);
    gpakWriteDsp (DspId, Address, NumWords, (DSP_Word *) DlWordBufr);
    gpakReadDsp  (DspId, Address, NumWords, (DSP_Word *) DlByteBufr);
	//print_hex_dump_bytes("DUMP DLWORDBUFFER LE  - ", 2,DlWordBufr, NumWords);
}
