...
#include <pika/si3215_registers.h>
...
void daytona_pll_fxs(PDEVICE_EXTENSION pdx)
{
	...
	fpga_write(pdx->bar0, BAR0_RESET, 0x39);

	mdelay(1);

	/* Take PLL out of reset */
	fpga_write(pdx->bar0, BAR0_RESET, 0x3B);
...
EXPORT_SYMBOL(daytona_phone_disconnect);
...
void phone_tasklet(unsigned long arg)
{
	...
	rt_mask = fpga_read(pdx->bar0, BAR0_RNG_TRP_STAT);
	if (rt_mask)
		fpga_write(pdx->bar0, BAR0_RNG_TRP_STAT, ~rt_mask); /* clear */
	lc_mask = fpga_read(pdx->bar0, BAR0_LC_TRANS_STAT);
	if (lc_mask)
		fpga_write(pdx->bar0, BAR0_LC_TRANS_STAT, ~lc_mask); /* clear */
#endif
