			   ...
			   u8 ind_reg_offset , u16 data)
{
	...
	fpga_write_indirect_locked(fpga, channel, I_DATA_LOW,  (u8)(data & 0x00FF));
	fpga_write_indirect_locked(fpga, channel, I_DATA_HIGH, (u8)(data >> 8));
	fpga_write_indirect_locked(fpga, channel, I_ADDRESS,   ind_reg_offset);
...
int read_silabs(PDEVICE_EXTENSION pdx, unsigned trunk, unsigned reg, u8 *out)
{
	...
	if (wait_for_ciar(pdx))
		return -ETIMEDOUT;
	fpga_write(pdx->bar0, BAR0_CIAR, val);
	if (wait_for_ciar(pdx))
		return -ETIMEDOUT;
	data = fpga_read(pdx->bar0, BAR0_CIAR);
	*out = data;
	return 0;
}
...
static inline int wait_for_fiar(PDEVICE_EXTENSION pdx)
{
	 ...
	 */
	while ((fpga_read(pdx->bar0, BAR0_STAT_LINES) & 0x2000) == 0) {
		if (++count > 100000) {
...
int read_flash(PDEVICE_EXTENSION pdx, u8 *flash)
{
	...
	/* Bank 1 read */
	fpga_write(pdx->bar0, BAR0_FIAR, 0x00410000);
