...
void imr_set(PDEVICE_EXTENSION pdx, unsigned set)
{
	...
	set &= ~IMR_MASK;
	fpga_write(pdx->bar0, BAR0_IMR, imr | set);
	pdx->imr |= set;
}
...
void imr_clr(PDEVICE_EXTENSION pdx, unsigned clr)
{
	unsigned imr = fpga_read(pdx->bar0, BAR0_IMR);
	clr &= ~IMR_MASK;
	fpga_write(pdx->bar0, BAR0_IMR, imr & ~clr);
	pdx->imr &= ~clr;
}
...
void daytona_reset(PDEVICE_EXTENSION pdx)
{
	...
	val = fpga_read(pdx->bar0, BAR0_ICCR);
	val &= ~IMR_MASK;
	fpga_write(pdx->bar0, BAR0_ICCR, val);

	/* Disable DMA. */
	fpga_write(pdx->bar0, BAR0_DEC_DMA_CTRL, 0);
	...
	for (reg = BAR0_ABV_THRSH_STAT; reg <= BAR0_HKFLSH_STAT; reg += 4)
		fpga_write(pdx->bar0, reg, 0);

	pdx->ring_head = pdx->ring_tail = 0;

	/* Just in case the diag was set... */
	val = fpga_read(pdx->bar0, BAR0_DIAG);
	fpga_write(pdx->bar0, BAR0_DIAG, val & 1);
...
int daytona_init(PDEVICE_EXTENSION pdx)
{
	...
	prod = fpga_read(pdx->bar0, BAR0_FPGA_REV);
	if (prod & (1 << 17)) {
		prod = fpga_read(pdx->bar0, BAR0_EXP_PRODUCT_ID);
		if (prod == 9900840)
			fxo_mask |= exp_mask;
...
int daytona_silabs_reset(PDEVICE_EXTENSION pdx)
{
	...
	config = fpga_read(pdx->bar0, BAR0_CONFIG);
	config &= ~1;
	fpga_write(pdx->bar0, BAR0_CONFIG, config);
#endif
	...
	reset = fpga_read(pdx->bar0, BAR0_RESET);
...
#ifdef FPGA_6052
	reset &= ~FPGA_RESET_PRI_BIT;
#endif

#endif // CONFIG_WARP
	reset |= FPGA_RESET_SPI_BIT; /* Needed for GW and WARP */
	fpga_write(pdx->bar0, BAR0_RESET, reset);

	udelay(10); // PRI needs 10us from 3us orginal value.
...
void daytona_pll_fxo(PDEVICE_EXTENSION pdx)
{
	...
	fpga_write(pdx->bar0, BAR0_PLL_CONTROL, 1);

	val = fpga_read(pdx->bar0, BAR0_RESET);

	/*  Take PLL out of rst */
	val |= 0x2;
	fpga_write(pdx->bar0, BAR0_RESET, val);
...
int daytona_configure_fxo(PDEVICE_EXTENSION pdx, unsigned mask)
{
	...
	trunk_for_each(i) {
		/* The PCM is divided as:  0-31 for Module A, 32-64 for Module B
		   For module B (>=8) we offset to the second half of the PCM stream by adding 256.
		   Then we can treat phones 8-11 as though they are 0-3, so that's why we subtract 8 from the phone number.
		   The | of 1 is to offset by 1 bit compared to the start of the 
		   frame pulse.
		 */
#ifdef FPGA_6052
		if ( i >= 8 )
	...
	if (dbg)
		PK_PRINT("%d: daytona_init done. FPGA_REV %x\n",
			 pdx->id, fpga_read(pdx->bar0, BAR0_FPGA_REV));
...
int daytona_get_info(PDEVICE_EXTENSION pdx)
{
	...
	pdx->info.FPGA_version = fpga_read(pdx->bar0, BAR0_FPGA_REV) & 0xffff;
...
void do_trunks(PDEVICE_EXTENSION pdx, unsigned iccr)
{
	...
	if (iccr & 0xfff) {
		...
		for (i = 0; i < ICCR_TRUNK_INTS; ++i, set <<= 1, reg -= 4) {
			if (iccr & set) {
				stat_mask = fpga_read(pdx->bar0, reg);
				PK_PRINT("do_trunks reg: %x = 0x%08x\n", reg, stat_mask );
				fpga_write(pdx->bar0, reg, ~stat_mask);
...
void do_phones(PDEVICE_EXTENSION pdx, unsigned iccr)
{
	...
	if (iccr & 0x03000000) { /* Ring trip or LC */
		...
		if (iccr & 0x01000000) { /* Ring trip */
			stat_mask = fpga_read(pdx->bar0, BAR0_RNG_TRP_STAT);
			PK_PRINT("BAR0_RNG_TRP_STAT: 0x%08x\n", stat_mask);
			fpga_write(pdx->bar0, BAR0_RNG_TRP_STAT, ~stat_mask);
			pdx->rt_mask |= stat_mask;
		}
		if (iccr & 0x02000000) { /* LC */
			stat_mask = fpga_read(pdx->bar0, BAR0_LC_TRANS_STAT);
			PK_PRINT("BAR0_LC_TRANS_STAT: 0x%08x\n", stat_mask);
			fpga_write(pdx->bar0, BAR0_LC_TRANS_STAT, ~stat_mask);
...
int daytona_set_detect(PDEVICE_EXTENSION pdx, u32 trunk, u32 mask)
{
	...
	imr = fpga_read(pdx->bar0, BAR0_IMR);
	mask = imr & ~0xFDC;
	for (i = 0; i < ICCR_MAX_LINES; ++i)
		mask |= pdx->line_imr[i];
	if (mask != imr) {
		pdx->imr = mask & ~IMR_MASK;
		fpga_write(pdx->bar0, BAR0_IMR, mask);
		if (dbg)
...
int daytona_hook_switch(PDEVICE_EXTENSION pdx, trunk_hookswitch_t *rqst)
{
	...
	case TRUNK_GO_HOOKFLASH:
		...
		fpga_write(pdx->bar0, BAR0_HKFLSH_START, (1 << trunk));
		break;
	case TRUNK_STOP_HOOKFLASH:
		fpga_write(pdx->bar0, BAR0_HKFLSH_STOP, (1 << trunk));
		break;
	case TRUNK_STOP_DIALING:
		fpga_write(pdx->bar0, BAR0_STOP_DIAL, (1 << trunk));
		break;
	default:
		return -EINVAL;
	}
...
int daytona_trunk_info(PDEVICE_EXTENSION pdx, u32 trunk, PKH_TTrunkInfo *info)
{
	u8 data;
...
int daytona_trunk_dial(PDEVICE_EXTENSION pdx, trunk_dial_t *rqst)
{
	...
	for (i = 0; i < 32; ++i, reg += 4)
		fpga_write(pdx->bar0, reg, rqst->string[i]);
	...
	/* Start dialing... */
	fpga_write(pdx->bar0, BAR0_START_DIAL, (1 << rqst->trunk));
...
int daytona_phone_config(PDEVICE_EXTENSION pdx, u32 phone, u32 intCtrl, u32 audioFormat)
{
	u8 data;
...
int daytona_phone_start(PDEVICE_EXTENSION pdx, u32 phone)
{
	...
	fpga_write(pdx->bar0, 0x124, 0);
	fpga_write(pdx->bar0, 0x120, 0);
...
int daytona_phone_info(PDEVICE_EXTENSION pdx, int phone, phone_get_info_out_t *info)
{
	u8 data;
...
static void clock_set_master(PDEVICE_EXTENSION pdx)
{
	...
	/* Set PLL to freerun */
	val = fpga_read(pdx->bar0, BAR0_PLL_CONTROL) | 1;
	fpga_write(pdx->bar0, BAR0_PLL_CONTROL, val);

	/* Wait for PLL to actually be in freerun for upto 1ms */
	for (count = 0; count < 1000; ++count) {
		if (fpga_read(pdx->bar0, BAR0_STAT_LINES) & 0x20000)
			break;
		udelay(1);
	}

	/* Hold the recovery in reset. */
	val = fpga_read(pdx->bar0, BAR0_RESET) | 0x10;
	fpga_write(pdx->bar0, BAR0_RESET, val);
}
...
static void clock_set_slave(PDEVICE_EXTENSION pdx)
{
	...
	fpga_write(pdx->bar0, BAR0_PLL_CONTROL, 0x9);
	...
	fpga_write(pdx->bar0, BAR0_RESET, val | 0x10);
	fpga_write(pdx->bar0, BAR0_RESET, val & ~0x10);

	/* remove pll from freerun */
	fpga_write(pdx->bar0, BAR0_PLL_CONTROL, 0x8);
	/* Enable PLL ints */
...
void daytona_clock_tick(void)
{
	...
	list_for_each_entry(pdx, &board_list, list) {
		...
		case CLOCK_SET_SLAVE:
			...
			break;
		case CLOCK_SLAVE:
			fpga_write(pdx->bar0, BAR0_SYS_TICK, 0xFEED);
			break;
		}
	}
}
...
void daytona_clock_reset(void)
{
	...
	list_for_each_entry(pdx, &board_list, list) {
		if (pdx->clock_mode == CLOCK_SLAVE) {
			unsigned val = fpga_read(pdx->bar0, BAR0_RESET);
